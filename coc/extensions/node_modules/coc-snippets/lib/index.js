...
function registerLanguageProvider(subscriptions, channel) {
  const configuration = import_coc.workspace.getConfiguration("snippets");
  const trace = configuration.get("trace", "error");
  let languageProvider = new LanguageProvider(channel, trace);
  subscriptions.push(import_coc.languages.registerCompletionItemProvider("snippets-source", configuration.get("shortcut", "S"), ["snippets"], languageProvider, ["$"], configuration.get("priority", 90)));
}
...
// src/index.ts
async function editSnippets(text) {
...
async function activate(context) {
  ...
  if (configuration.get("ultisnips.enable", true)) {
    ...
    subscriptions.push(import_coc7.commands.registerCommand("snippets.editSnippets", editSnippets));
  }
